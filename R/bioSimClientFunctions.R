...
getNormals <- function(period, variables, id, latDeg, longDeg, elevM, averageOverTheseMonths) {
  ...
  maps <- J4R::callJavaMethod("biosimclient.BioSimClient", "getNormals", jPeriod, jVariables, jPlots, jAverageOverTheseMonths)
...
getAnnualNormals <- function(period, variables, id, latDeg, longDeg, elevM) {
  return(getNormals(period, variables, id, latDeg, longDeg, elevM, BioSIM::allMonths))
}
